...
import { clearSelection, setTool, setViewMode, zoomIn, zoomOut, resetZoom, ViewMode } from '@/features/editor/editorSlice';
import { undo, redo, removeFurnitureFromDesign, addFurnitureToDesign, updateFurnitureInDesign } from '@/features/design/designSlice';
...
export const useKeyboardShortcuts = (options: UseKeyboardShortcutsOptions = {}) => {
  ...
  const selectedFurnitureIds = useSelector((state: RootState) => state.editor.selectedFurnitureIds);
  ...
  const nudgeSelected = useCallback(
    (dx: number, dy: number) => {
      ...
      selectedFurnitureIds.forEach((id) => {
        ...
        const newY = Math.max(
          0,
          Math.min(currentDesign.room.length - furnitureData.dimensions.length, item.position.y + dy)
        );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!ctrl && e.key === '1') {
        e.preventDefault();
        dispatch(setViewMode('2d' as ViewMode));
        return;
      }
      if (!ctrl && e.key === '3') {
        e.preventDefault();
        dispatch(setViewMode('3d' as ViewMode));
        return;
      }
