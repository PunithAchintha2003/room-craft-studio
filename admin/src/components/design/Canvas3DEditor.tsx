...
const InteractiveFurniture: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleChange = () => {
      ...
      onTransform(item.id, {
        position: { x: group.position.x - halfW, y: group.position.z - halfL },
    ...
    <>
      ...
      >
        <Suspense fallback={null}>
          <FurnitureModel3D furnitureItem={item} furniture={furniture} isSelected={isSelected} onClick={onClick} />
        </Suspense>
      </group>
...
const Scene: React.FC<{
  ...
  return (
    <>
      ...
      <hemisphereLight intensity={lightingConfig.hemisphereIntensity} color="#ddeeff" groundColor="#443322" />
      <directionalLight
        position={sunPos}
        ...
        shadow-bias={-0.0005}
        color={
          lightingConfig.timeOfDay === 'evening'
            ? '#FFB74D'
            : lightingConfig.timeOfDay === 'morning'
              ? '#FFCC80'
              : '#FFF9F0'
        }
      />
      ...
      {lightingConfig.shadowsEnabled && (
        <ContactShadows position={[0, 0.001, 0]} opacity={0.35} scale={roomDiag * 2.5} blur={2.5} far={8} color="#000020" />
      )}

      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]} onClick={handleGroundClick} visible={false}>
...
export const Canvas3DEditor: React.FC<Canvas3DEditorProps> = React.memo(
  ({ design, furniture, width = '100%', height = '100%', onGroundClick, lightingConfig }) => {
      ...
      (id: string) => {
        dispatch(
          setSelectedFurnitureIds(
            selectedFurnitureIds.includes(id) ? selectedFurnitureIds.filter((fid) => fid !== id) : [id]
          )
        );
      },
    ...
    return (
      <Box sx={{ width, height, position: 'relative', bgcolor: '#0F1117', borderRadius: 1, overflow: 'hidden' }}>
        <Box sx={{ position: 'absolute', top: 12, right: 12, zIndex: 10, display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          <Stack direction="column" spacing={0.5} sx={{ bgcolor: 'rgba(20,20,30,0.85)', backdropFilter: 'blur(8px)', borderRadius: 1, p: 0.5 }}>
        ...
        <Chip
          label={`Camera: ${cameraPreset}`}
          size="small"
          sx={{ position: 'absolute', bottom: 12, left: 12, zIndex: 10, bgcolor: 'rgba(20,20,30,0.75)', color: '#ccc', fontSize: 11, backdropFilter: 'blur(6px)' }}
        />
        ...
        {selectedFurnitureIds.length > 0 && (
          <Chip
            ...
            size="small"
            sx={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', zIndex: 10, bgcolor: 'rgba(20,20,30,0.75)', color: '#aaa', fontSize: 10, backdropFilter: 'blur(6px)' }}
          />
        )}
        ...
        {webglLost && (
          <Box sx={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'rgba(10,10,14,0.72)', backdropFilter: 'blur(6px)', zIndex: 30, p: 2, textAlign: 'center' }}>
            <Box>
              <Typography variant="h6" sx={{ mb: 1, color: 'grey.100' }}>
                3D renderer reset
              </Typography>
              <Typography variant="body2" sx={{ color: 'grey.300', maxWidth: 520 }}>
                Your browser lost the WebGL context (usually GPU/driver memory pressure). Try closing other heavy tabs, then refresh the page.
              </Typography>
            </Box>
          </Box>
        )}
