...
export const Canvas2DEditor: React.FC<Canvas2DEditorProps> = ({ width = 800, height = 600 }) => {
  const dispatch = useDispatch();
  const stageRef = useRef<Konva.Stage>(null);
  const transformerRef = useRef<Konva.Transformer>(null);
  const [localSnapGuides, setLocalSnapGuides] = useState<{
    x: number | null;
    y: number | null;
    xLabel?: string;
    yLabel?: string;
  ...
  const renderRoom = () => {
    ...
    if (layout === 'rectangle') {
      return (
        <Group>
          ...
          <Rect
            x={0}
            y={roomLengthPx - WALL_THICKNESS_PX}
            width={roomWidthPx}
          ...
          <Rect
            x={roomWidthPx - WALL_THICKNESS_PX}
            y={0}
    ...
    return (
      <Group>
        <Line points={points} closed fill={currentDesign.room.floorColor || '#F5E6D0'} listening={false} />
        <Line points={points} closed stroke={wallColor} strokeWidth={WALL_THICKNESS_PX} fill="transparent" lineJoin="miter" listening={false} />
        <Line points={points} closed stroke="#424242" strokeWidth={1.5} fill="transparent" listening={false} />
        <Text x={roomWidthPx / 2 - 20} y={roomLengthPx + 8} text={`${currentDesign.room.width}m`} fontSize={11} fill="#616161" listening={false} />
        <Text x={roomWidthPx + 8} y={roomLengthPx / 2 - 8} text={`${currentDesign.room.length}m`} fontSize={11} fill="#616161" listening={false} />
      </Group>
    );
  ...
  const computeSnap = useCallback(
    (itemId: string, rawPxX: number, rawPxY: number, itemPxW: number, itemPxH: number): { x: number; y: number } => {
      ...
      const targetsX = computeSnapTargetsX(currentDesign.room.width, PIXELS_PER_METER, currentDesign.furniture, furniture, itemId);
      const targetsY = computeSnapTargetsY(currentDesign.room.length, PIXELS_PER_METER, currentDesign.furniture, furniture, itemId);
  ...
  const handleDragMove = (id: string, e: Konva.KonvaEventObject<DragEvent>, itemPxW: number, itemPxH: number) => {
    const node = e.target;
    const rawX = node.x();
    const rawY = node.y();
  ...
  const handleDragEnd = (id: string, e: Konva.KonvaEventObject<DragEvent>, furnitureWidth: number, furnitureLength: number) => {
    const node = e.target;
  ...
  const handleTransformEnd = (id: string, e: Konva.KonvaEventObject<Event>) => {
    const node = e.target;
    dispatch(updateFurnitureInDesign({ id, updates: { rotation: node.rotation() % 360, scale: Math.max(0.5, Math.min(2.0, node.scaleX())) } }));
    node.scaleX(1);
    node.scaleY(1);
  };
  ...
  return (
    <Paper elevation={0} sx={{ width, height, overflow: 'hidden', position: 'relative', bgcolor: 'transparent', borderRadius: 1 }}>
      <Stage
        width={width}
        height={height}
        ref={stageRef}
          ...
          {currentDesign.furniture.map((item) => {
            ...
            return (
              <Canvas2DFurnitureItem
                ...
                onDragMove={(fid, ev) => handleDragMove(fid, ev, itemPxW, itemPxH)}
                onDragEnd={(fid, ev) => handleDragEnd(fid, ev, furnitureData.dimensions.width, furnitureData.dimensions.length)}
