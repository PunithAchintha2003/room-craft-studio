...
export const LightingControlPanel: React.FC<LightingControlPanelProps> = ({ config, onChange }) => {
  ...
  const applyTimeOfDayPreset = (time: LightingConfig['timeOfDay']) => {
    const presets: Record<LightingConfig['timeOfDay'], Partial<LightingConfig>> = {
      morning: {
  ...
  const handleTimeOfDayChange = (
    ...
    newValue: LightingConfig['timeOfDay'] | null
  ) => {
    if (!newValue) return;
