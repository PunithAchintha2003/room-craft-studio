...
export const DesignEditorPage: React.FC = () => {
  ...
  const { currentDesign, loading, saving, error, history, historyIndex } = useSelector(
    (state: RootState) => state.design
  );
  const { furniture } = useSelector((state: RootState) => state.furniture);
  const { showGrid, zoom, tool, viewMode, showRuler, showAlignmentGuides, snapToGrid, showCeiling } = useSelector(
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast.error(message);
      if (message.toLowerCase().includes('log in')) navigate('/login');
    }
  }, [currentDesign, dispatch, navigate]);
    ...
    if (id) {
      dispatch(fetchDesigns()).then((action) => {
        if (fetchDesigns.fulfilled.match(action)) {
          const design = action.payload.find((d: Design) => d._id === id);
          if (design) {
  ...
  const handleBack = () => {
    if (hasUnsavedChanges) {
      if (window.confirm('You have unsaved changes. Leave anyway?')) navigate('/my-designs');
    } else {
      navigate('/my-designs');
    }
  };
  ...
  if (error) {
    return (
      <Box p={3}>
        ...
        <Button onClick={() => navigate('/my-designs')} sx={{ mt: 2 }}>
          Back to My Designs
        </Button>
      </Box>
    );
  ...
  return (
    ...
    >
      <Card sx={{ position: 'sticky', top: 0, zIndex: 2, overflow: 'hidden' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.25, py: 1, flexWrap: 'wrap' }}>
          <Tooltip title="Back to My Designs">
            <IconButton edge="start" onClick={handleBack} size="small" sx={{ color: 'text.secondary' }}>
            ...
            {(Object.entries(VIEW_MODE_CONFIG) as ['2d' | '3d', typeof VIEW_MODE_CONFIG['2d']][]).map(
              ([mode, { label, icon }]) => (
                <ToggleButton
                  key={mode}
                  value={mode}
                  {...(mode === '3d' ? { 'data-tour': '3d-preview' as const } : {})}
                >
          ...
          <Tooltip title="Undo (Cmd+Z)">
            <span>
              <IconButton onClick={() => dispatch(undo())} disabled={!canUndo} size="small" sx={{ color: canUndo ? 'text.secondary' : alpha(theme.palette.text.secondary, 0.35) }}>
          ...
          <Tooltip title="Redo (Cmd+Shift+Z)">
            <span>
              <IconButton onClick={() => dispatch(redo())} disabled={!canRedo} size="small" sx={{ color: canRedo ? 'text.secondary' : alpha(theme.palette.text.secondary, 0.35) }}>
          ...
          {viewMode === '2d' && (
            <>
              <Tooltip title="Zoom In (Ctrl+=)">
                <IconButton onClick={() => dispatch(zoomIn())} disabled={zoom >= 3} size="small" sx={{ color: 'text.secondary' }}>
                  <ZoomInIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title="Zoom Out (Ctrl+-)">
                <IconButton onClick={() => dispatch(zoomOut())} disabled={zoom <= 0.3} size="small" sx={{ color: 'text.secondary' }}>
                  <ZoomOutIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title={showGrid ? 'Hide Grid' : 'Show Grid'}>
                <IconButton data-tour="grid-toggle" onClick={() => dispatch(toggleShowGrid())} size="small" sx={{ color: showGrid ? theme.palette.primary.main : 'text.secondary' }}>
                  {showGrid ? <GridOffIcon fontSize="small" /> : <GridOnIcon fontSize="small" />}
                </IconButton>
              </Tooltip>
              <Tooltip title={showRuler ? 'Hide Ruler' : 'Show Ruler'}>
                <IconButton onClick={() => dispatch(toggleShowRuler())} size="small" sx={{ color: showRuler ? theme.palette.primary.main : 'text.secondary' }}>
                  <RulerIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title={showAlignmentGuides ? 'Disable Snap Guides' : 'Enable Snap Guides'}>
                <IconButton onClick={() => dispatch(toggleAlignmentGuides())} size="small" sx={{ color: showAlignmentGuides ? theme.palette.success.main : 'text.secondary' }}>
                  <MagnetIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title={snapToGrid ? 'Disable Grid Snap' : 'Enable Grid Snap'}>
                <IconButton onClick={() => dispatch(toggleSnapToGrid())} size="small" sx={{ color: snapToGrid ? theme.palette.warning.main : 'text.secondary' }}>
                  <WeekendIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title={tool === 'measure' ? 'Exit Measure (M)' : 'Measure Tool (M)'}>
                <IconButton onClick={() => dispatch(setTool(tool === 'measure' ? 'select' : 'measure'))} size="small" sx={{ color: tool === 'measure' ? theme.palette.secondary.main : 'text.secondary' }}>
          ...
          {viewMode === '3d' && (
            <>
              <Tooltip title="Lighting Settings">
                <IconButton onClick={() => setLightingDrawerOpen(true)} size="small" sx={{ color: lightingDrawerOpen ? theme.palette.warning.main : 'text.secondary' }}>
                  <WbSunnyIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title={showCeiling ? 'Hide Ceiling' : 'Show Ceiling'}>
                <IconButton onClick={() => dispatch(toggleShowCeiling())} size="small" sx={{ color: showCeiling ? theme.palette.info?.main ?? theme.palette.primary.light : 'text.secondary' }}>
                  <View3DIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title="Export">
                <IconButton onClick={() => setExportDialogOpen(true)} size="small" sx={{ color: exportDialogOpen ? theme.palette.primary.main : 'text.secondary' }}>
          ...
          <Tooltip title="Help (?)">
            <IconButton onClick={() => setHelpDialogOpen(true)} size="small" sx={{ color: 'text.secondary' }} data-tour="help-button">
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '320px 1fr 340px' }, gap: 2, flex: 1, minHeight: 0 }}>
        <Card
          sx={{
            display: 'flex',
            flexDirection: 'column',
            overflow: 'hidden',
            minHeight: 0,
            '& .MuiPaper-root': { backgroundImage: 'none', backgroundColor: 'transparent', boxShadow: 'none', border: 'none' },
          }}
        ...
        >
          ...
          <Box sx={{ flex: 1, overflowY: 'auto', px: 2, pb: 2, scrollbarWidth: 'none', msOverflowStyle: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>
        ...
        <Card
          sx={{
            ...
            backgroundColor: theme.palette.mode === 'dark' ? GLASS.dark.background : GLASS.light.background,
        ...
        >
          <Box
            ref={canvas2DRef}
            sx={{ position: 'absolute', inset: 0, display: viewMode === '2d' ? 'flex' : 'none', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', borderRadius: 1 }}
            data-tour="canvas-2d"
          >
            <Canvas2DEditor width={canvas2DSize.width - 8} height={canvas2DSize.height - 4} />
          </Box>

          <Box ref={canvas3DRef} sx={{ position: 'absolute', inset: 0, display: viewMode === '3d' ? 'block' : 'none', overflow: 'hidden', borderRadius: 1 }}>
            <Suspense fallback={<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}><CircularProgress /></Box>}>
              <Canvas3DEditor design={currentDesign} furniture={furniture} width="100%" height="100%" lightingConfig={lightingConfig} />
            </Suspense>
          </Box>
        </Card>
        ...
        >
          <Box sx={{ px: 2, py: 1.5, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.7)}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
            <Typography variant="overline" sx={{ color: 'text.secondary' }}>
              Furniture
            </Typography>
            <Button
              ...
              sx={{ minHeight: 30, px: 1.25, py: 0.25, fontSize: 12, borderColor: alpha(theme.palette.divider, 0.9), color: 'text.secondary', '&:hover': { borderColor: 'primary.main', color: 'primary.main' } }}
            >
              Properties
            </Button>
          </Box>
      ...
      <Drawer
        ...
        hideBackdrop
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2, py: 2, borderBottom: `1px solid ${(theme.palette.mode === 'dark' ? GLASS.dark : GLASS.light).border}` }}>
          <Typography variant="overline" sx={{ color: 'text.secondary' }}>
            Properties
          </Typography>
      ...
      <Drawer
        ...
        PaperProps={{
          sx: { width: 280, borderLeft: `1px solid ${alpha(theme.palette.divider, 0.7)}`, mt: 0, height: '100%' },
        }}
        hideBackdrop
      >
      ...
      <Dialog open={saveDialogOpen} onClose={() => setSaveDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" label="Design Name" fullWidth value={designName} onChange={(e) => setDesignName(e.target.value)} sx={{ mb: 2 }} />
          <TextField margin="dense" label="Description (optional)" fullWidth multiline rows={2} value={designDescription} onChange={(e) => setDesignDescription(e.target.value)} />
        </DialogContent>
        <DialogActions>
